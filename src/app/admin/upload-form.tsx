...
export default function AdminUploadForm() {
  ...
  const confirmDelete = async () => {
    ...
    const toastId = toast.loading(deleteTarget.type === 'all' ? "Menghapus database..." : "Menghapus data...");
  ...
  const clearAllRecords = () => { if (confirm("Hapus semua di tabel review?")) setReviewData([]); };
      ...
      <section className="flex-1 flex flex-col min-w-0 max-h-screen">
         <header className="flex h-16 md:h-20 shrink-0 items-center justify-between bg-white px-4 md:px-8 border-b border-zinc-200">
            <div className="flex items-center gap-3">
               ...
               <h2 className="text-sm md:text-lg font-black text-zinc-900 uppercase tracking-tight truncate">{activeTab === 'portal' ? "Konfigurasi" : "Data Management"}</h2>
            </div>
                          ...
                          <tbody className="divide-y divide-zinc-100">
                             {isAddingManual && (
                               <tr className="bg-primary/5 animate-in slide-in-from-top-1 duration-200">
                                  ...
                                  <td className="px-4 py-2"><select value={newManualForm.status} onChange={(e) => setNewManualForm({...newManualForm, status: e.target.value as any})} className="w-full rounded-lg border border-zinc-200 p-2 text-[10px] font-black outline-none"><option value="LULUS">LULUS</option><option value="TIDAK LULUS">TIDAK LULUS</option></select></td>
                             ...
                             {isFetchingDb ? (<tr><td colSpan={5} className="py-20 text-center"><i className="ri-loader-4-line animate-spin text-4xl text-zinc-200" /></td></tr>) : dbStudents.length === 0 ? (<tr><td colSpan={5} className="py-20 text-center text-zinc-400 font-bold uppercase text-xs">Kosong</td></tr>) : (() => {
                                ...
                                return displayed.map((student) => {
                                   ...
                                   <tr key={student._id} className="hover:bg-zinc-50 transition-colors">
                                      ...
                                      <td className="px-6 py-4">{isEditing ? (<select value={editForm?.status} onChange={(e) => setEditForm(prev => prev ? {...prev, status: e.target.value as any} : null)} className="w-full rounded-lg border border-zinc-200 p-1.5 text-[10px] font-black outline-none"><option value="LULUS">LULUS</option><option value="TIDAK LULUS">TIDAK LULUS</option></select>) : (<span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${student.status === 'LULUS' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>{student.status}</span>)}</td>
                    ...
                    <tbody>
                       {reviewData.map((student, idx) => (
                          <tr key={idx} className="group">
                             ...
                             <td className="bg-white p-2 border-y border-zinc-100 shadow-sm"><select value={student.status} onChange={(e) => updateRecord(idx, "status", e.target.value as any)} className="w-full rounded-xl bg-zinc-50 px-3 py-1.5 text-[10px] font-black uppercase outline-none"><option value="LULUS">LULUS</option><option value="TIDAK LULUS">TIDAK LULUS</option></select></td>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-zinc-900/90 backdrop-blur-sm p-4 animate-in fade-in duration-200">
           <div className="w-full max-w-md rounded-[2.5rem] bg-white p-8 md:p-12 shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <div className="text-center space-y-3">
                 ...
                 <p className="text-sm font-medium text-zinc-500 leading-relaxed">{deleteTarget?.type === 'all' ? "Hapus SELURUH data? Tindakan ini permanen." : <>Hapus data siswa <span className="font-black text-zinc-900">"{deleteTarget?.name}"</span>?</>}</p>
              </div>
